...
C 语言课程设计项目。本仓库为**完整实现版本**，已完成车辆、员工、客户、销售记录、销售报表和二进制数据持久化功能。
...
- Windows 下推荐使用 `mingw32-make`，详细环境配置见 `Windows环境配置指南.pdf`
- 若使用 VS Code 的 CMake Tools，也应选择 MinGW 对应 Kit
...
- `*.dat` 为程序内部使用的二进制文件，不建议手工编辑
- 退出系统时会自动保存全部数据，也可通过主菜单“保存所有数据”手动保存
- 若程序被强制关闭，未保存的本次修改可能丢失
- 二进制文件格式依赖当前结构体定义；如果后续修改结构体字段，旧数据文件可能不兼容
...
4. 销售管理 → 添加：
   ...
   客户编号 C0001 → 自动显示 李四
   总价自动算出 360000
5. 需要中途落盘时：主菜单 → 保存所有数据
6. 销售报表 → 按型号/经手人汇总，导出 report.txt
7. 退出系统 → 自动保存全部数据
```
